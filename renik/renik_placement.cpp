...
void RenIKPlacement::interpolate_transforms(float fraction, bool update_hips,
                                            bool update_feet) {
  if (update_hips) {
  ...
  if (update_feet) {
    interpolated_left_foot =
        prev_left_foot.interpolate_with(target_left_foot, fraction);
    interpolated_right_foot =
        prev_right_foot.interpolate_with(target_right_foot, fraction);
  }
}

void RenIKPlacement::hip_place(float delta, Transform3D head,
  ...
  Vector3 foot_direction =
      (foot - head.origin).project(foot_median - head.origin);
  ...
  Vector3 follow_hip_direction =
      target_hip.basis.xform_inv(head.basis.xform(hip_offset));
...
*/
void RenIKPlacement::foot_place(float delta, Transform3D head, Ref<World3D> w3d,
                                bool instant) {
  ...
  Vector3 leftStop = head.origin +
                     Vector3(0,
                             (-spine_length - left_leg_length - floor_offset) *
                                 ...
                                 left_hip_offset[1],
                             0) +
                     head.basis.xform(left_hip_offset);
  Vector3 rightStop =
      head.origin +
      Vector3(0,
...
Transform3D RenIKPlacement::dangle_foot(Transform3D head, float distance,
                                        float leg_length, Vector3 hip_offset) {
  Transform3D foot;
  Basis upright_head =
      RenIKHelper::align_vectors(Vector3(0, 1, 0), head.basis[1])
...
void RenIKPlacement::initialize_loop(Vector3 velocity, Vector3 left_ground,
  ...
  if (left_grounded && right_grounded) {
    ...
    if (dot == 0) {
      float left_dist =
          target_left_foot.origin.distance_squared_to(left_ground);
      float right_dist =
          target_right_foot.origin.distance_squared_to(right_ground);
...
int RenIKPlacement::get_loop_state(float loop_state_scaling,
                                   float loop_progress,
                                   float &loop_state_progress, Gait gait) {
  int state = -1;
  float ground_time = gait.ground_time - gait.ground_time * loop_state_scaling;
  float lift_time =
      gait.lift_time_base + gait.lift_time_scalar * loop_state_scaling;
  float apex_in_time =
      gait.apex_in_time_base + gait.apex_in_time_scalar * loop_state_scaling;
  float apex_out_time =
      gait.apex_out_time_base + gait.apex_out_time_scalar * loop_state_scaling;
  float drop_time =
      gait.drop_time_base + gait.drop_time_scalar * loop_state_scaling;
...
void RenIKPlacement::loop_foot(Transform3D &step, Transform3D &stand,
                               ...
                               Node3D **prev_ground, int &loop_state,
                               Vector3 &grounded_stop, Transform3D head,
                               float leg_length, float foot_length,
                               Vector3 velocity, float loop_scaling,
                               float step_progress, Vector3 ground_pos,
  ...
  loop_state =
      get_loop_state(loop_scaling, step_progress, loop_state_progress, gait);
  float head_distance = head.origin.distance_to(ground_pos);
  float ease_scaling = loop_scaling * loop_scaling * loop_scaling *
                       loop_scaling; // ease the growth a little
  ...
  Transform3D grounded_foot =
      Transform3D(head.basis * upright_foot, ground_pos);
  Transform3D lifted_foot = Transform3D(
      grounded_foot.basis.rotated_local(Vector3(1, 0, 0),
                                        ease_scaling * gait.lift_angle),
      ground_pos +
          ground_normal * vertical_scaling * gait.lift_vertical_scalar +
          ground_normal * head_distance * gait.lift_vertical -
          ground_velocity.normalized() * horizontal_scaling *
              gait.lift_horizontal_scalar);
  Transform3D apex_foot = Transform3D(
      grounded_foot.basis.rotated_local(Vector3(1, 0, 0),
                                        ease_scaling * gait.apex_angle),
      ground_pos +
          ground_normal * vertical_scaling * gait.apex_vertical_scalar +
          ground_normal * head_distance * gait.apex_vertical);
  Transform3D drop_foot = Transform3D(
      grounded_foot.basis.rotated_local(Vector3(1, 0, 0),
                                        ease_scaling * gait.drop_angle),
      ground_pos +
          ground_normal * vertical_scaling * gait.drop_vertical_scalar +
          ground_normal * head_distance * gait.drop_vertical_scalar +
          ground_velocity.normalized() * horizontal_scaling *
              gait.drop_horizontal_scalar);
  ...
  case LOOP_GROUND_OUT: {
    ...
    float tip_toe_angle = step_distance * gait.tip_toe_distance_scalar +
                          horizontal_scaling * gait.tip_toe_speed_scalar;
    tip_toe_angle = tip_toe_angle > gait.tip_toe_angle_max
                        ? gait.tip_toe_angle_max
                        : tip_toe_angle;
  ...
  case LOOP_LIFT: {
    ...
    step.origin = grounded_stop.cubic_interpolate(
        lifted_foot.origin,
        grounded_stop - ground_velocity * horizontal_scaling,
        lifted_foot.origin + ground_normal * vertical_scaling,
        loop_state_progress);
    break;
  }
  case LOOP_APEX_IN:
    step.basis = lifted_foot.basis.slerp(apex_foot.basis, loop_state_progress);
    step.origin = lifted_foot.origin.cubic_interpolate(
        apex_foot.origin, lifted_foot.origin - ground_normal * vertical_scaling,
        apex_foot.origin + ground_velocity * leg_length, loop_state_progress);
    break;
  case LOOP_APEX_OUT:
    step.basis = apex_foot.basis.slerp(drop_foot.basis, loop_state_progress);
    step.origin = apex_foot.origin.cubic_interpolate(
        drop_foot.origin,
        apex_foot.origin - ground_velocity * horizontal_scaling,
        drop_foot.origin - ground_normal * vertical_scaling,
        loop_state_progress);
    break;
  case LOOP_DROP:
    step.basis =
        drop_foot.basis.slerp(grounded_foot.basis, loop_state_progress);
    step.origin = drop_foot.origin.cubic_interpolate(
  ...
  if (loop_state != LOOP_GROUND_IN && loop_state != LOOP_GROUND_OUT) {
    ...
    if (walk_state != LOOP_LIFT) {
      ...
      float contact_easing = gait.contact_point_ease +
                             gait.contact_point_ease_scalar * loop_scaling;
...
void RenIKPlacement::loop(Transform3D head, Vector3 velocity,
                          Vector3 left_ground_pos, Vector3 left_normal,
                          Vector3 right_ground_pos, Vector3 right_normal,
  ...
  float new_loop_scaling =
      max_threshold > min_threshold
          ...
          : 0;
  loop_scaling = (loop_scaling * gait.scaling_ease +
                  new_loop_scaling * (1 - gait.scaling_ease));
  step_progress =
      Math::fmod((step_progress +
                  stride_speed * (gait.speed_scalar_min * (1 - loop_scaling) +
                                  gait.speed_scalar_max * loop_scaling)),
                 1.0f);

  if (left_grounded) {
    loop_foot(left_step, left_stand, left_stand_local, left_ground,
              &prev_left_ground, left_loop_state, left_grounded_stop, head,
              left_leg_length, left_foot_length, velocity, loop_scaling,
              step_progress, left_ground_pos, left_normal, gait);
  } else {
    Transform3D left_dangle =
        dangle_foot(head, (spine_length + left_leg_length) * dangle_ratio,
  ...
  if (right_grounded) {
    loop_foot(right_step, right_stand, right_stand_local, right_ground,
              &prev_right_ground, right_loop_state, right_grounded_stop, head,
              right_leg_length, right_foot_length, velocity, loop_scaling,
              Math::fmod((step_progress + 0.5f), 1.0f), right_ground_pos,
              right_normal, gait);
  } else {
    Transform3D right_dangle =
        dangle_foot(head, (spine_length + right_leg_length) * dangle_ratio,
...
void RenIKPlacement::step_direction(Vector3 forward, Vector3 side,
                                    Vector3 velocity, Vector3 left_ground,
                                    Vector3 right_ground, bool left_grounded,
                                    bool right_grounded) {
  ...
  if (Math::abs(normalized_velocity.dot(normalized_side)) >
    ...
    if (walk_state != STRAFING && walk_state != STRAFING_TRANSITION) {
      ...
      initialize_loop(normalized_velocity, left_ground, right_ground,
...
void RenIKPlacement::stand_foot(Transform3D foot, Transform3D &stand,
                                Transform3D &stand_local, Node3D *ground) {
  Transform3D ground_global = ground->get_global_transform();
  ground_global.basis.orthonormalize();
...
void RenIKPlacement::foot_place(
    float delta, Transform3D head,
    PhysicsDirectSpaceState3D::RayResult left_raycast,
    PhysicsDirectSpaceState3D::RayResult right_raycast,
    PhysicsDirectSpaceState3D::RayResult laying_raycast, bool instant) {
  ...
  if (left_raycast.collider != nullptr) {
    left_velocity =
        RenIKHelper::vector_rejection(velocity, left_raycast.normal);
  } else {
    left_velocity = RenIKHelper::vector_rejection(velocity, Vector3(0, 1, 0));
  }
  if (right_raycast.collider != nullptr) {
    right_velocity =
        RenIKHelper::vector_rejection(velocity, right_raycast.normal);
  } else {
  ...
  if ((!left_raycast.collider && !right_raycast.collider &&
       !laying_raycast.collider) ||
      fall_override) {
    ...
    case STANDING: { // brackets to stop declarations from breaking case labels
      ...
      Vector3 left_head_forward =
          (head.basis *
           RenIKHelper::align_vectors(
               Vector3(0, 1, 0), head.basis.xform_inv(left_raycast.normal)))[2];
      Vector3 right_head_forward =
          (head.basis * RenIKHelper::align_vectors(
                            Vector3(0, 1, 0),
                            head.basis.xform_inv(right_raycast.normal)))[2];
      ...
      if (left_velocity.length() > effective_min_threshold ||
          ...
          (left_raycast.collider != nullptr &&
           left_stand.origin.distance_squared_to(left_raycast.position) >
               balance_threshold * (left_leg_length + right_leg_length) / 2) ||
          (right_raycast.collider != nullptr &&
           right_stand.origin.distance_squared_to(right_raycast.position) >
               balance_threshold * (left_leg_length + right_leg_length) / 2) ||
          (left_raycast.collider != nullptr &&
           (Node3D *)left_raycast.collider != left_ground) ||
          (right_raycast.collider != nullptr &&
           (Node3D *)right_raycast.collider != right_ground) ||
          left_head_forward.dot(left_forward) < cos(rotation_threshold) ||
          right_head_forward.dot(right_forward) < cos(rotation_threshold) ||
          (left_raycast.collider != nullptr &&
           left_raycast.normal.dot(left_upward) < cos(rotation_threshold) &&
           upward.dot(left_upward) < cos(rotation_threshold)) ||
          (right_raycast.collider != nullptr &&
           right_raycast.normal.dot(right_upward) < cos(rotation_threshold) &&
           upward.dot(right_upward) < cos(rotation_threshold))) {
        step_direction(forward, feet_sideways, velocity, left_raycast.position,
                       right_raycast.position, left_raycast.collider != nullptr,
                       right_raycast.collider != nullptr);
      }
      break;
    }
  ...
  case STEPPING: {
    ...
    effective_transition_progress = effective_transition_progress <= 1
                                        ? effective_transition_progress
                                        : 1.0;
    loop(head, velocity, left_raycast.position, left_raycast.normal,
         right_raycast.position, right_raycast.normal,
         left_raycast.collider != nullptr, right_raycast.collider != nullptr,
         forward_gait);
    target_left_foot =
...
void RenIKPlacement::set_falling(bool falling) { fall_override = falling; }
