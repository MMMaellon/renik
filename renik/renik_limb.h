...
public:
  ...
  float twist_inflection_point_offset =
      0; // When the limb snaps from twisting in the positive direction to
         // twisting in the negative direction
  float twist_overflow =
      0; // How much past the inflection point we go before snapping

  Quaternion
      pole_offset; /*ADVANCED - Moving the limb 180 degrees from rest tends to
  ...
  Vector3
      target_position_influence; // ADVANCED - How much each of the leaf's axis
                                 // of translation from rest affects the ik
  float target_rotation_influence; // ADVANCED - How much the rotation the leaf
                                   // points in affects the ik

  // STATE: We're keeping a little bit of state now... kinda goes against the
  // design, but it makes life easier so fuck it
  int overflow_state =
...
struct RenIKLimbSubSection : public Resource {
  ...
  float joint_bend_direction_offset =
      0; // change the axis of the joint bend without changing the twist of the
         // bone
  float twist_offset = 0; // change the twist of the bone without changing the
                          // axis along which the joint bends
