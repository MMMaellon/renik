...
#define RENIK_PROPERTY_STRING_HAND_RIGHT_TARGET_PATH                           \
  "armature_right_hand_target"
...
#define RENIK_PROPERTY_STRING_FOOT_RIGHT_TARGET_PATH                           \
  "armature_right_foot_target"
...
void RenIK::apply_ik_map(Map<BoneId, Quaternion> ik_map,
...
void RenIK::apply_ik_map(Map<BoneId, Transform3D> ik_map,
                         Transform3D global_parent,
                         Vector<BoneId> apply_order) {
  if (skeleton) {
    Transform3D global_pose = global_parent;
    for (int i = 0; i < apply_order.size(); i++) {
      global_pose *=
          skeleton->get_bone_rest(apply_order[i]) * ik_map[apply_order[i]];
      skeleton->set_bone_global_pose_override(apply_order[i], global_pose, 1.0);
    }
  }
}
...
RenIK::SpineTransforms RenIK::perform_torso_ik() {
  if (head_target_spatial && skeleton && spine_chain->is_valid()) {
    ...
    skeleton->set_bone_global_pose_override(
        hip, inverse_skeleton * hipGlobalTransform, 1.0f);

    apply_ik_map(ik_map, inverse_skeleton * hipGlobalTransform,
                 bone_id_order(spine_chain));

    // Keep Hip and Head as global poses tand then apply them as global pose
    // override
    skeleton->set_bone_global_pose_override(
        head, inverse_skeleton * headGlobalTransform, 1.0f);
...
void RenIK::perform_hand_left_ik(Transform3D global_parent) {
  if (hand_left_target_spatial && skeleton &&
      ...
      if (left_shoulder_enabled) {
        ...
        Quaternion quatAlignToTarget =
            ...
            RenIKHelper::align_vectors(
                Vector3(0, 1, 0), poleOffset.inverse().xform(
                                      offsetQuat.inverse().xform(targetVector)))
                .slerp(Quaternion(), 1 - shoulder_influence);
        Transform3D customPose =
            Transform3D(offsetQuat * quatAlignToTarget, Vector3());
        skeleton->set_bone_global_pose_override(rootBone, root * customPose,
                                                1.0f);
    ...
    apply_ik_map(
        solve_trig_ik_redux(limb_arm_left, root,
                            hand_left_target_spatial->get_global_transform()),
...
void RenIK::perform_hand_right_ik(Transform3D global_parent) {
  if (hand_right_target_spatial && skeleton &&
    ...
    apply_ik_map(
        solve_trig_ik_redux(limb_arm_right, root,
                            hand_right_target_spatial->get_global_transform()),
...
void RenIK::perform_foot_left_ik(Transform3D global_parent) {
  if (skeleton && limb_leg_left->is_valid_in_skeleton(skeleton)) {
    ...
    if (foot_left_target_spatial) {
      ...
      // BoneId rootBone =
      // skeleton->get_bone_parent(limb_leg_left->get_upper_bone()); if
      // (rootBone >= 0) { 	root = root *
      // skeleton->get_bone_global_pose(rootBone);
      // }
      apply_ik_map(
          solve_trig_ik_redux(limb_leg_left, root,
                              foot_left_target_spatial->get_global_transform()),
      ...
      // skeleton->get_bone_global_pose(rootBone);
      // }
      apply_ik_map(solve_trig_ik_redux(limb_leg_left, root,
                                       placement.interpolated_left_foot),
                   global_parent, bone_id_order(limb_leg_left));
    }
  }
}
...
void RenIK::perform_foot_right_ik(Transform3D global_parent) {
  if (skeleton && limb_leg_right->is_valid_in_skeleton(skeleton)) {
    ...
    if (foot_right_target_spatial) {
      ...
      // skeleton->get_bone_global_pose(rootBone);
      // }
      apply_ik_map(solve_trig_ik_redux(limb_leg_right, root,
                                       placement.interpolated_right_foot),
                   global_parent, bone_id_order(limb_leg_right));
    }
  }
}
...
Map<BoneId, Quaternion> RenIK::solve_trig_ik(Ref<RenIKLimb> limb,
  ...
  if (limb->is_valid()) { // There's no way to find a valid upperId if any of
    ...
    //  old way: Quaternion lowerTwist = (align_vectors(lowerVector,
    //  leafRest.xform(leaf.xform(lowerVector))).inverse() * (leafRest *
...
std::pair<float, float> RenIK::trig_angles(Vector3 const &side1,
  ...
  float angle2 =
      Math_PI - RenIKHelper::safe_acos(
                    (length1Squared + length2Squared - length3Squared) /
                    (length1 * length2));
  return std::make_pair(angle1, angle2);
}

Map<BoneId, Transform3D> RenIK::solve_trig_ik_redux(Ref<RenIKLimb> limb,
                                                    Transform3D root,
                                                    Transform3D target) {
  ...
  if (limb->is_valid()) {
    ...
    Transform3D upperTransform = Transform3D(
        (full_upper.get_basis().inverse() * upperBasis).orthonormalized());
    Transform3D lowerTransform = Transform3D(
        (full_lower.get_basis().inverse() * lowerBasis).orthonormalized());
    Transform3D leafTransform =
        Transform3D(limb->get_leaf().get_basis().inverse() *
                    (upperBasis * lowerBasis).inverse() *
                    localTarget.get_basis() * limb->get_leaf().get_basis());
    ...
    map[limb->get_lower_bone()] =
        lowerTransform; // limb->upper_extra_bones.affine_inverse() *
                        // (full_lower.get_basis().inverse() * lowerBasis);
...
Map<BoneId, Quaternion> RenIK::solve_ifabrik(Ref<RenIKChain> chain,
  ...
  if (chain->is_valid()) { // if the chain is valid there's at least one joint
    ...
    // Quaternion pose = align_vectors(axis, Transform3D(parentRot,
    // parent.origin).affine_inverse().xform(target.origin)); //offset by one
    // because joints has one extra element map.insert(joints[0].id, pose);
    // parentRot = parentRot * pose;
  }
  return map;
}
